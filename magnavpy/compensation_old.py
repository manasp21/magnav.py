...
def nn_comp_2_fwd(
    ...
    y_hat_norm_np = y_hat_norm_torch.cpu().clone().detach().numpy()
...
def nn_comp_2_train(
    ...
    for i in range(1, epoch_adam + 1):
        ...
        for A_b_norm, x_b_norm, y_b_norm in train_loader:
            ...
            sgl_term = torch.tensor(0.0, device=loss_train_val.device, requires_grad=True)
            if lambda_ > 0:
    ...
    if l_segs is not None:
        valid_l_segs_final = l_segs if len(l_segs)>0 and sum(l_segs) == len(y_f32) else [len(y_f32)]
    else:
        valid_l_segs_final = [len(y_f32)]
...
def comp_train_df( # Overload for DataFrames, renamed from comp_train
    ...
    Axy_outputs = get_Axy_fn(
        ...
        sub_igrf=sub_igrf,
        bpf_mag_data_in_x=bpf_mag,
        reorient_vec=reorient_vec,
        mod_TL=(model_type == "mod_TL"),
        map_TL=(model_type == "map_TL"),
        return_B_comps=model_type.startswith("m3"), # Only M3 explicitly needs Bt, B_dot from get_Axy
    ...
    if isinstance(comp_params, NNCompParams) and comp_params.drop_fi:
        ...
        if model_type == "m1" and isinstance(comp_params, NNCompParams):
            ...
            model_res, tl_coef_res, dn_res, y_hat_np, err_np = nn_comp_2_train(
                ...
                comp_params.frac_train, comp_params.α_sgl, comp_params.λ_sgl, comp_params.k_pca,
                data_norms_in=data_norms, model_in=model, l_segs=l_segs_out,
                A_test_in=A_test_for_train, x_test_in=x_test_for_train, y_test_in=y_test_for_train, l_segs_test=l_segs_test_for_train, silent=silent
            )
            comp_params.model = model_res
            comp_params.TL_coef = tl_coef_res
            comp_params.data_norms = dn_res
